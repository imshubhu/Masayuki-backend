...
app.get("/delete", async (req, res, next) => {
  try {
    ...
    const getImageData = await uploadModel.findOne({_id: req.query.id}).lean().exec();

    for (let i = 0; i < getImageData.images.length; i++) {
      const element = getImageData.images[i];
